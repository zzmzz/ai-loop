...
- **持久记忆** -- CLAUDE.md 累积记忆 + 滑动窗口压缩，跨轮次保持上下文连贯；包版本升级时自动刷新各角色模板正文并保留 `## 累积记忆` 以下内容
- **产品认知库** -- `.ai-loop/product-knowledge/` 由 Product 维护（索引 + 按业务域拆分的 Markdown），探索/验收 prompt 会注入 `index.md` 便于跨轮恢复产品上下文
- **结构化日志** -- `.ai-loop/logs/round-*.jsonl` 记录阶段切换、AI 调用与结果统计、Brain 决策及 CLI 用户交互，便于复盘与排错
