...
**滑动窗口与摘要压缩**：随着轮次增多，累积记忆会膨胀。`limits.memory_window`（默认 5）控制保留最近 N 轮的完整记忆，更早的轮次由 Brain 压缩为一段"历史摘要"（`compact_memories`）。这确保 CLAUDE.md 不会无限膨胀，同时保留关键历史上下文。

**角色专属记忆**：每轮结束时，Brain 会为 product / developer / reviewer 三个角色生成差异化的记忆摘要。产品经理侧重需求和验收结论，开发者侧重技术决策和实现细节，审查者侧重代码质量和风险点。

### 上下文管理

`ContextCollector` 负责在每个阶段自动注入前序产物。例如，开发者进入实现阶段时，`ContextCollector` 会自动将 `design.md` 和 `clarification.md`（如有）的内容内联注入到 prompt 中，无需角色手动读取文件。

每个阶段的依赖关系在 `ContextCollector.PHASE_DEPS` 中定义，避免了重复文件读取和 token 浪费。

### 代码理解缓存

每轮结束后，系统自动生成 `code-digest.md` —— 一份项目代码结构和关键变更的摘要。下一轮的产品探索阶段会以增量阅读模式工作：先读取缓存的 digest，只在需要深入了解时才读取具体源文件。这减少了每轮开始时的重复代码探索开销。
...
AI Loop 支持三种项目类型，不同类型使用不同的验收策略：

| 类型 | 验收方式 | 适用场景 |
|------|----------|----------|
| `web` | Playwright 浏览器自动化 | Web 应用、有 UI 的项目 |
| `cli` | 运行示例命令 + 测试套件 | 命令行工具 |
| `library` | 运行测试套件 | SDK、库、框架 |
...
project:
  name: MyApp                    # 必填：项目名称
  path: .                        # 必填：项目路径（支持相对路径，相对于 .ai-loop 的父目录解析）
...
verification:                    # 必填：验收策略
  type: web                      # "web" | "cli" | "library"
  base_url: http://localhost:3000  # web 类型必填：浏览器访问地址

server:                          # web 类型建议配置
  start_command: npm run dev     # 必填：启动命令
  health_url: http://localhost:3000  # 必填：健康检查 URL
  start_cwd: .                   # 可选，默认 "."
  health_timeout: 30             # 可选，默认 30（秒）
  stop_signal: SIGTERM           # 可选，默认 "SIGTERM"

browser:                         # 可选（向后兼容，推荐用 verification.base_url 替代）
...
> **向后兼容**：如果只配置了 `browser.base_url` 而没有 `verification`，会自动视为 `verification.type: web`。
