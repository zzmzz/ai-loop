...
| 决策 | 含义 | 后续动作 |
|------|------|----------|
| `PROCEED` | 实现完整 | 进入 QA 验收 |
| `RETRY` | 有遗漏 | Developer 补完 |

### post_acceptance

- **触发时机**：Product 完成 QA 测试 + 验收后
- **输入文件**：`requirement.md`, `acceptance.md`
- **指令**：评估 QA 测试与验收结果（acceptance.md 包含需求验证、探索发现和健康评分）
- **可选决策**：

| 决策 | 含义 | 后续动作 |
|------|------|----------|
| `PASS` | 全部通过，健康评分 ≥ 80 | 轮次完成 |
| `PARTIAL_OK` | P0 全过，无 Critical | 轮次完成（下轮处理） |
| `FAIL_IMPL` | P0 未过或有 Critical 发现 | Developer 修复 → 重新验收 |
...
- **触发时机**：验收通过后
- **输入文件**：`requirement.md`, `design.md`, `dev-log.md`, `acceptance.md`
- **指令**：生成本轮总结 + 角色专属记忆
- **输出 JSON schema**：
